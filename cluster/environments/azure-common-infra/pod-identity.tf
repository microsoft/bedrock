...
module "pod_identity_kv_role" {
  ...
  service_principal_object_id = "${module.pod_identity.pod_msi_principal_id}"
  subscription_id             = "${data.azurerm_client_config.current.subscription_id}"
  precursor_done              = "${module.pod_identity.pod_msi_client_id}"
}
...
module "pod_identy_kv_access" {
  ...
  resource_group_name         = "${data.azurerm_resource_group.global_rg.name}"
  keyvault_name               = "${module.keyvault.keyvault_name}"
  service_principal_object_id = "${module.pod_identity.pod_msi_client_id}"
  key_permissions             = ["get"]
