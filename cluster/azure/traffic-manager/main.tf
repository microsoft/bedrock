...
resource "null_resource" "traffic_manager" {
  ...
  provisioner "local-exec" {
    command = "${path.module}/create_traffic_manager.sh -s ${var.dns_subscription_id} -g ${var.resource_group_name} -t ${var.traffic_manager_name} -e \"${var.service_names}\" -u \"${var.service_suffix}\" -z ${var.dns_zone_name} -p \"${var.probe_path}\""
  }
