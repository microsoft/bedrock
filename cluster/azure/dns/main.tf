...
resource "null_resource" "dnszone" {
  ...
  provisioner "local-exec" {
    command = "${path.module}/ensure_dns_zone.sh -s ${var.subscription_name} -g ${var.resource_group_name} -z ${var.name} -c ${var.caa_issuer} -o ${var.service_principal_object_id} -e ${var.env_name}"
  }
