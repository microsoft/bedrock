...
resource "null_resource" "create_k8s_secret" {
  ...
  provisioner "local-exec" {
    command = "echo 'Need to use this var so terraform waits for kubeconfig ' ${var.kubeconfigadmin_done};KUBECONFIG=${var.output_directory}/${var.kubeconfigadmin_filename} ${path.module}/create_k8s_secret.sh -v ${var.key_vault_name} -s ${var.key_vault_secret_name} -n ${var.k8s_secret_name} -m \"${var.k8s_namespaces}\""
  }
