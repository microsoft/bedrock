...
This walkthrough uses an Azure DevOps pipeline and Fabrikate to automate changes to a running Bedrock deployment. A deployment such as [A Walkthrough Deploying a Bedrock Environment](https://github.com/MikeDodaro/bedrock-deploy-demo) can assign the `gitops_ssh_url` field in the `terraform.tfvars` file to a repo that receives the output of Fabrikate in a DevOps pipeline.  

When the pipeline runs, it calls `fab install` and `fab generate`, which we ran from the commandline in a previous example: [Build Fabrikate Definition for Container Deployment](https://github.com/MikeDodaro/bedrock-deploy-demo/tree/master/fabrikate).  The manifest that results from the DevOps pipeline is sent to another repo and directory that Flux is watching.  Updates to the Bedrock deployment are automatic.

## Set up git repos for Pipeline

You'll need two repos for this example, one to run Fabrikate and another to receive the resulting `.yaml` files.  We're using the `azure-vote` application also used in [Build Fabrikate Definition for Container Deployment](https://github.com/MikeDodaro/bedrock-deploy-demo/tree/master/fabrikate).  

### Fabrikate repo
In this example, the repo that runs Fabrikate in the DevOps pipeline uses a directory structure like the following screenshot.  For now, add a [simple web app manifest](https://github.com/MikeDodaro/bedrock-deploy-demo#deploy-an-update-using-kubernetes-manifest) to the `vote-deploy` directory.
...
The Bedrock deployment that will be updated by manifests from this repo must authenticate and authorized Flux to query this repo.  From the [Bedrock deployment](https://github.com/MikeDodaro/bedrock-deploy-demo#create-an-rsa-key-pair-for-a-deploy-key-for-the-flux-repository), use `ssh-keygen` to generate a deploy key.  Add it to this repo as described in the deployment document under the heading [Add Repository Key](https://github.com/MikeDodaro/bedrock-deploy-demo#add-repository-key).  
...
If you deployed [A Walkthrough Deploying a Bedrock Environment](https://github.com/MikeDodaro/bedrock-deploy-demo), it is simple to target the output of this pipeline in the `terraform.tfvars` file of the deployment.  
...
Test this scenario by pushing a new manifest to the `vote-deploy` directory of the Fabrikate repo.  Use the `azure-vote.yaml` manifest from [Build Fabrikate Definition for Container Deployment](https://github.com/MikeDodaro/bedrock-deploy-demo/tree/master/fabrikate.)

Next, follow the instructions under [Results](https://github.com/MikeDodaro/bedrock-deploy-demo/tree/master/fabrikate#results) to find the `EXTERNAL-IP` of a new load balancer that will appear when Flux picks up the change and it is deployed to the cluster.  
